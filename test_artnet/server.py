...
# Named Tuple of time, universe, and DMX data
ArtNetData = namedtuple('ArtNetData', ['time', 'universe', 'data'])
...
class ArtNetRecorder(asyncio.DatagramProtocol):
    ...
    def add_data(self, universe, data):
        self.data_list.append(ArtNetData(time.time(), universe, data))
    ...
    def datagram_received(self, data, addr):
        if data.startswith(ART_NET_HEADER):
            ...
            if opcode == b'\x00P':
                ...
                dmx_data = data[18:]
                print(
                    f"Received Art-Net data from {addr[0]}:{addr[1]} for Universe {universe}")
                self.add_data(universe, dmx_data)

    async def receive_artnet_data(self):
        loop = asyncio.get_running_loop()
        self.transport, _ = await loop.create_datagram_endpoint(
            lambda: self, local_addr=('127.0.0.1', ART_NET_PORT)
        )
